...
description: Um tipo de referência que é marcado especificamente como visível para Component Object Model (COM) contém um construtor público com parâmetros, mas não contém um construtor público padrão (sem parâmetros).
ms.date: 11/04/2016
ms.topic: reference
f1_keywords:
