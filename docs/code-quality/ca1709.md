...
description: O nome de um identificador não está de forma correta, contém um acrônimo de duas letras e a segunda letra é minúscula ou contém um acrônimo de três ou mais letras maiúsculas.
ms.date: 11/04/2016
ms.topic: reference
f1_keywords:
...
|Alteração da falha|Quebra-quando gerado em assemblies, namespaces, tipos, membros e parâmetros.<br /><br /> Não separável-quando acionado em parâmetros de tipo genérico.|
