---
title: 'Tutorial: Edição para desenvolvedores de C#'
...
ms.locfileid: "112390249"
---
# <a name="learn-to-use-the-code-editor-with-c"></a>Saiba como usar o editor de código com C\#

Nesta introdução de 10 minutos ao editor de códigos no Visual Studio, adicionaremos código a um arquivo para ver algumas das maneiras pelas quais o Visual Studio facilita a escrita, a navegação e a compreensão do código C#.
...
1. Coloque o cursor logo acima da chave de fechamento **final }** no arquivo e digite os caracteres (que significa não se preocupe muito se você não sabe o que `svm` isso `static void Main` &mdash; significa).
...
Os snippets de código disponíveis variam em linguagens de programação diferentes. Você pode ver os snippets de código disponíveis para sua linguagem escolhendo Editar Snippet de Inserção do  >  **IntelliSense** ou pressionando  >   **Ctrl** + **K,** **Ctrl** + **X** e, em seguida, escolhendo a pasta do idioma. Para o C#, a lista tem este aspecto:
...
O bloco de código é recolhido apenas na primeira linha, seguido por um sinal de reticências (`...`). Para expandir o bloco de código novamente, clique na mesma caixa cinza que agora tem um sinal de a mais ou pressione **Ctrl** + **M**, **Ctrl** + **M** novamente. Esse recurso é chamado [de Estrutura](../../ide/outlining.md) de Trabalho e é especialmente útil quando você está re colapsando métodos longos ou classes inteiras.

## <a name="view-symbol-definitions"></a>Exibir definições de símbolo

O Visual Studio editor facilita a inspeção da definição de um tipo, método etc. Uma maneira é navegar até o arquivo que contém a definição, por exemplo, escolhendo **Ir** para Definição ou pressionando **F12** em qualquer lugar em que o símbolo seja referenciado. Uma maneira ainda mais rápida que não move o foco para fora do arquivo em que você está trabalhando é usar a opção [Inspecionar Definição](../../ide/go-to-and-peek-definition.md#peek-definition). Vamos espiar a definição do tipo `string`.
...
[O IntelliSense](../../ide/using-intellisense.md) é um recurso valioso quando você está codificando. Ele pode mostrar informações sobre membros disponíveis de um tipo ou detalhes de parâmetros para sobrecargas diferentes de um método. Você também pode usar o IntelliSense para completar uma palavra depois que você digitar caracteres suficientes para desambiguá-la. Vamos adicionar uma linha de código para imprimir as cadeias de caracteres ordenadas na janela de console, que é o local padrão para envio da saída do programa.
...
1. Coloque o cursor sobre a definição da variável e escolha Renomear no menu de contexto ou clique com o botão direito do mouse ou pressione `_words` **Ctrl**  + **R,** **Ctrl** + **R**.
