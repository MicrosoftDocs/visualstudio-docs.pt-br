...
Você cria um projeto de um modelo usando **Arquivo**  >  **Novo**  >  **Projeto**. Para ver modelos para projetos Web, selecione **Python**  >  **Web** no lado esquerdo da caixa de diálogo. Em seguida, selecione um modelo de sua escolha, fornecendo nomes para o projeto e para a solução, defina opções para um diretório da solução e para o repositório Git e selecione **OK**.
...
O modelo genérico de **Projeto Web**, mencionado anteriormente, fornece apenas um projeto vazio do Visual Studio sem nenhum código e nenhuma suposição diferente além de ser um projeto do Python.
...
Todos **os modelos de Projeto \<framework> Web** em Branco criam um projeto com um código clichê mais ou menos mínimo e as dependências necessárias declaradas em um *arquivorequirements.txt* aplicativo.
...
| **Projeto Web do Flask/Jade** | Gera o mesmo aplicativo que o modelo **projeto Web flask,** mas usando a extensão Jade para o mecanismo de emplatação Jinja. |

::: moniker range="vs-2017"
### <a name="polls-group"></a>Grupo de pesquisas

Os **modelos do Projeto Web \<framework> pesquisas** criam um aplicativo Web inicial por meio do qual os usuários podem votar em diferentes perguntas de sondagem. Cada aplicativo se baseia na estrutura dos modelos de projeto **Web** para usar um banco de dados para gerenciar as pesquisas e as respostas do usuário. Os aplicativos incluem modelos de dados adequados e uma página de aplicativo especial (/seed) que carrega votações de um arquivo *samples.json*.
...
| **Pesquisas Projeto Web do Flask/Jade** | Gera o mesmo aplicativo que o modelo **Pesquisas projeto Web flask,** mas usando a extensão Jade para o mecanismo de emplatação Jinja. |
::: moniker-end
...
- **Ambiente:** uma nova lista separada por linha de pares \<NAME> = \<VALUE> que especificam variáveis de ambiente. Essas variáveis são definidas após todas as propriedades que podem modificar o ambiente, como o número da porta e os caminhos de pesquisa e, portanto, podem substituir esses valores.
...
`$(StartupFile)` é o caminho relativo para o arquivo de inicialização e `{StartupModule}` é o nome importável do arquivo de inicialização. `$(SERVER_HOST)`e `$(SERVER_PORT)` são variáveis de ambiente normais  definidas pelas propriedades **URL** de Início e Número da Porta, automaticamente ou **pela propriedade** Ambiente.
