...
description: Um tipo tem segurança declarativa e um de seus métodos tem segurança declarativa para a mesma ação de segurança, e a ação de segurança não é LinkDemand, e as permissões verificadas pelo tipo não são um subconjunto das permissões verificadas pelo método.
ms.date: 11/04/2016
ms.topic: reference
f1_keywords:
